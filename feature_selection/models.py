...
def perm_knn(X,y, action = None):
    ...
    perm_plot = save_plot(X.columns, importance, 'perm_knn.png')
    return perm_plot 

 
def chi_2(X_train, y_train, X_test):
    # feature selection
    ...
    # plot the scores
    chi2_plot = save_plot(X_train.columns, fs.scores_, 'chi_2.png')


def mutual_inf(X_train, y_train, X_test):
    # feature selection
    ...
    for i in range(len(fs.scores_)):
        print('Feature %d: %f' % (i, fs.scores_[i]))
    # plot the scores
    mutualinf_plot = save_plot(X_train.columns, fs.scores_, 'mutual_inf.png')
...
def categorical_corr(df):
    ...
    save_plot_sns(corr, 'categorical_correlation.png')
...
def unc_coeff(df):
    ...
    corr = pd.DataFrame(corrM, index=cols, columns=cols)
    save_plot_sns(corr, 'uncertainty_coefficients.png')
